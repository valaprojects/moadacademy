  ...
  return (
    <motion.article className="neon-hover group relative overflow-hidden rounded-[26px] border border-black/6 bg-white p-2 shadow-[0_16px_50px_rgba(17,22,14,.05)]" initial={false} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ delay: index * .04, duration: .5 }}>
      ...
      <div className="relative aspect-[1.08/1] overflow-hidden rounded-[21px] p-5 text-white" style={{ background: `radial-gradient(circle at 25% 20%, ${pack.accent}22, transparent 35%), linear-gradient(145deg, ${pack.color}, #080a08)` }}>
        ...
        <div className="absolute inset-x-5 bottom-5"><Waveform color={pack.accent} /><div className="mt-3 text-[19px] font-black leading-tight">{pack.title}</div><div className="mt-2 flex items-center justify-between text-[9px] text-white/45"><span>{pack.genre}</span><span>{pack.bpm}</span></div></div>
        <button type="button" onClick={() => setPlaying(!playing)} className="absolute left-5 top-14 z-20 grid size-11 place-items-center rounded-full border border-white/15 bg-white/10 backdrop-blur-xl transition hover:scale-105" aria-label={playing ? "توقف پیش نمایش" : "پخش پیش نمایش"}>{playing ? <Pause className="size-4" fill="currentColor" /> : <Play className="mr-0.5 size-4" fill="currentColor" />}</button>
      </div>
